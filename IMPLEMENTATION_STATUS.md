...
## 進捗更新（PPTX比較：4-1 図形拡張）
- 図形拡張を実装
  - `index.html`
    - オブジェクト追加パネルへ `線` / `矢印` / `角丸矩形` / `菱形` の追加ボタンを導線
  - `app.js`
    - `addElement` に `line` / `arrow` / `roundedRect` / `diamond` を追加
    - `renderElement` に各新規図形描画処理を追加（線／矢印は座標幅高さでベクトル描画）
    - `buildArrowHeadPoints` を追加し、矢印先端ポリゴンを生成して線端に描画
    - `getElementBounds` は既存 x/y/width/height 前提の形状バウンディングに準拠させた実装のまま利用
    - `setupEvents` と `dom` に新規ボタン/イベント接続
    - `renderProperties` の選択ラベルに新規図形名を反映
- 受け入れ条件
  - 追加した図形が既存の移動・リサイズ・整列・複数選択・重ね順に対応する
  - クリックでの追加が可能で、キャンバス上に即時描画される
- 追加対応（矢印向き調整）
  - `app.js`
    - `line` / `arrow` 向きを回転角度で制御するための新規プロパティ `rotation` を追加
    - `renderProperties` で `向き（度）` を追加し、`line`/`arrow` のみ表示して更新可能にした
    - `getLineEndpoints` を追加し、`rotation` を反映した線端点計算で描画
...
- スライド一覧確認と切り替えのUIを改善
  - `index.html`
    - `スライド操作` に「前へ」「次へ」ボタンと `#slide-jump`（スライド直接選択）を追加
    - 既存一覧（`#slide-list`）はそのまま保持
  - `app.js`
    - `goToSlide` を追加し、前後移動・一覧クリック・直接選択を共通制御
    - `renderSlideList` を拡張し、`slide-jump` の選択肢更新と前後ボタンの活性状態管理を実装
    - `Ctrl/Cmd + 左右矢印` でスライド前後移動を追加
  - `styles.css`
    - `slide-navigation` のグリッドレイアウト追加（ボタン＋セレクトを1行で整理）
